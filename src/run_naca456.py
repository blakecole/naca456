...
if __name__ == "__main__":
    # executed from project-root/src
    naca = NACA456()
    ...
    result = naca.generate(namelist, preview=True)
    
    if len(result)==3: print(" Cambered profile with ", len(result[0]), " points.")
    else: print(" Symmmetric profile with ", len(result[0]), " points.")
    #print(f"{len(x)} points → ../out/{fname}.out")